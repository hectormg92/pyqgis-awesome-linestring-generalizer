...
class GeneraLine(QtCore.QObject):
    ...
    def setLayer(self, layer):
        ...
        self.layer_salida = QgsVectorLayer( 'LineString?crs={}'.format(self.__crs)
                                        ...
                                        , 'memory' )
        print self.layer, self.layer_salida
    ...
    def  __douglasPeucker(self,points, epsilon):
        ...
        # Si el primer y el Ãltimo punto son el mismo
...
class MainWindow(QMainWindow):
    ...
    def guardaSHP(self):
        print self.simplify.layer_salida.name()
    ...
    def __init_ui(self):
        ...
        self.btn_zoom.setObjectName(_fromUtf8("Zoom extensiÃ³n"))
    ...
    def retranslateUi(self):
        ...
        self.tabs.setTabText(self.tabs.indexOf(self.tab_dp), _translate("MainWindow", "GeneralizaciÃ³n Douglas-Peucker", None))
        self.label_8.setText(_translate("MainWindow", "Î¼", None))
        ...
        self.btn_zoom.setText(_translate("MainWindow", "Zoom ExtensiÃ³n", None))
        ...
        self.label_shp_path.setText(_translate("MainWindow", "NingÃºn archivo SHP seleccionado", None))
...
class Runner(QtCore.QObject):
    ...
    def __init__(self, parent=None):
        ...
        self.worker = GeneraLine()
        self.worker_thread = QtCore.QThread()
        self.worker.moveToThread(self.worker_thread)
        self.worker_thread.start()

        self.window.btn_prev_dp.clicked.connect( partial(self.showPreviewCanvas, self.window.Algorithms.DOUGLAS_PEUCKER) )

        self.window.btn_prev_mcm.clicked.connect( partial(self.showPreviewCanvas, self.window.Algorithms.MC_MASTER) )

        self.window.btn_load_shp.clicked.connect(self.__openDialog)

        self.worker.signalStatus.connect(self.preview)

        self.window.show()

    def showPreviewCanvas(self, algorithm) :
        print algorithm
        if algorithm == self.window.Algorithms.DOUGLAS_PEUCKER :
            param = self.window.spn_dp.value()
        else :
            param = self.window.spn_mcm.value()

        print param

        self.worker.applyAlgorithm(algorithm, param)

    @QtCore.pyqtSlot()
    def preview(self):
        print 'preview'
        self.window.cb_suav.setEnabled(True)
        self.window.cb_suav.setCheckState(Qt.Checked)
        print 'ppp'
        self.window.addLayerToPreviewCanvas(self.worker.layer, self.worker.layer_salida)

    def __openDialog(self):
        dlg = QFileDialog()
        dlg.setFileMode(QFileDialog.ExistingFile)
        # Solo mostrar archivos con extensiÃÂÃÂ³n shp
        dlg.setFilter("shapefile (*.shp)")

        if dlg.exec_():
            # Obtenemos la ruta del archivo
            filepath = map(str, list(dlg.selectedFiles()))[0]
            self.addLayer(filepath)
    ...
    def addLayer(self, path):
        ...
        self.worker.setLayer(layer)

        self.window.canvas.setExtent(layer.extent())

        self.window.canvas.setLayerSet([self.original])

        symbols = layer.rendererV2().symbols()
        symbol = symbols[0]
        symbol.setColor(QtGui.QColor.fromRgb(204, 51, 51))

        self.window.btn_prev_dp.setEnabled(True)
        self.window.btn_prev_mcm.setEnabled(True)
        #self.cb_suav.setEnabled(True)
        self.window.cb_ori.setEnabled(True)
        self.window.cb_ori.setCheckState(Qt.Checked)
        self.window.cb_dp.setEnabled(True)
        self.window.cb_mcm.setEnabled(True)
        #self.cb_suav.setEnabled(False)
        self.window.btn_zoom.setEnabled(True)
        self.window.spn_dp.setEnabled(True)
        self.window.spn_mcm.setEnabled(True)
        self.window.scaleControl.setEnabled(True)


        self.window.canvas.mapRenderer().setDestinationCrs(QgsCoordinateReferenceSystem(layer.crs().authid()))

        self.window.cb_ori.toggled.connect( partial(self.window.toggleCanvasVisibilityLayer, self.window.original) )


        self.window.label_shp_path.setText(layer.name())

        self.window.btn_zoom.clicked.connect( partial( self.window.zoomExt, layer ) )
